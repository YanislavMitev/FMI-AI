...
public class DFSExample {
    ...
    private static Node getDestinationNodeFromTraversed(byte destinationX, byte destinationY) throws IllegalNodeException {
        ...
        if(destinationX >= matrix.length || destinationY >= matrix[0].length){
            throw new IllegalNodeException("Non-existing node! (Out of boundaries)\nThe algorithm cannot proceed.\nTerminating...");
        }
    ...
    /**
     * Function to display the path in the maze
     */
    private static void displayPathInMaze() {
        for(int row = 0; row < matrix.length; row++){
    ...
    public static void main(String[] args) {
        ...
        matrix[end.getXAxis()][end.getYAxis()] = 'F';
        ...
        while (true) {
            ...
            while (next != null) {
                matrix[next.getXAxis()][next.getYAxis()] = '|';
                if (next.equals(start)) {
                    matrix[next.getXAxis()][next.getYAxis()] = 'S';
